...
// each other's writes.
func newTestRepository(t *testing.T, ids id.Generator) (*postgres.ProviderRepository, context.Context) {
	t.Helper()
	...
	return postgres.NewProviderRepository(tx, clock.New(), ids), ctx
}

func testProvider(name string) provider.Provider {
...
func TestProviderRepositoryCreate(t *testing.T) {
	...
	if created.Name != "Acme Fiber" {
		t.Errorf("Name = %q, want %q", created.Name, "Acme Fiber")
	}
...
func TestProviderRepositoryCreatePersistsEachDefinedStatus(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())
...
func TestProviderRepositoryCreateIgnoresCallerSuppliedIdentity(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())

	bogusID := uuid.New()
	bogusTime := time.Date(1999, 1, 1, 0, 0, 0, 0, time.UTC)

	p := testProvider("Edge Provider")
	p.ID = bogusID
...
func TestProviderRepositoryGet(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())

	created, err := repo.Create(ctx, testProvider("Beta Fiber"))
	if err != nil {
		t.Fatalf("Create() = %v", err)
	}
...
func TestProviderRepositoryGetNotFound(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())
...
func TestProviderRepositoryList(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())

	first, err := repo.Create(ctx, testProvider("Alpha Fiber"))
	if err != nil {
		t.Fatalf("Create() = %v", err)
	}
	second, err := repo.Create(ctx, testProvider("Beta Fiber"))
	if err != nil {
		t.Fatalf("Create() = %v", err)
	}
	...
	found := make(map[uuid.UUID]provider.Provider, len(providers))
	for _, p := range providers {
		found[p.ID] = p
	}
	if _, ok := found[first.ID]; !ok {
		t.Error("List() did not include the first created provider")
	}
	if _, ok := found[second.ID]; !ok {
		t.Error("List() did not include the second created provider")
	}
	...
	if len(providers) != 2 {
		t.Fatalf("len(List()) = %d, want 2; got %+v", len(providers), providers)
	}
	if providers[0].Name != "Alpha Fiber" || providers[1].Name != "Beta Fiber" {
		t.Errorf("List() order = [%q, %q], want [Alpha Fiber, Beta Fiber]", providers[0].Name, providers[1].Name)
	}
}

func TestProviderRepositoryUpdate(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())
...
func TestProviderRepositoryUpdateNotFound(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())

	_, err := repo.Update(ctx, provider.Provider{
		ID:     uuid.New(),
		Name:   "Ghost",
		Status: provider.StatusActive,
	})
...
func TestProviderRepositoryDelete(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())

	created, err := repo.Create(ctx, testProvider("Temporary"))
	if err != nil {
		t.Fatalf("Create() = %v", err)
	}
...
func TestProviderRepositoryDeleteNotFound(t *testing.T) {
	repo, ctx := newTestRepository(t, id.New())
...
func TestProviderRepositoryCreateConflictOnDuplicateID(t *testing.T) {
	fixedID := uuid.New()
	repo, ctx := newTestRepository(t, id.Static{Value: fixedID})

	if _, err := repo.Create(ctx, testProvider("First")); err != nil {
		t.Fatalf("first Create() = %v", err)
	}

	_, err := repo.Create(ctx, testProvider("Second"))
