...
// newAuthenticatedTestRouter mounts ProviderHandler behind the real
...
// tests from provider_handler_test.go's: those test the handler in
// isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeProviderService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/providers", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireProvidersRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireProvidersWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeProviderService(), tokens, auth.RoleAdministrator)
...
// TestViewerCanReadProviders is "apply the standard RBAC matrix",
// applied to Providers: Viewer can read.
func TestViewerCanReadProviders(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProviderService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteProviders is "apply the standard RBAC matrix",
// applied to Providers: Viewer cannot write.
func TestViewerCannotWriteProviders(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProviderService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteProviders is "apply the standard RBAC matrix",
// applied to Providers: Operator can write.
func TestOperatorCanWriteProviders(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProviderService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteProviders is "apply the standard RBAC
// matrix", applied to Providers: Administrator can write.
func TestAdministratorCanWriteProviders(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProviderService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeProviderService(), expiredValidator, auth.RoleAdministrator)
