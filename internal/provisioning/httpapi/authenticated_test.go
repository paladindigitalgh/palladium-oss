...
// newAuthenticatedTestRouter mounts ProvisioningProfileHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/provisioning-profiles in
// production — using RequireCatalogRead/RequireCatalogWrite, the same
// capability pair /api/v1/products and /api/v1/catalogs use (see
// authz.CanReadCatalog's doc comment for why this domain does not get its
// own dedicated pair: a ProvisioningProfile only exists nested inside a
// Product, so all three resources answer the same access question). The
...
// provisioning_profile_handler_test.go's: those test the handler in
// isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeProvisioningProfileService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/provisioning-profiles/", nil))
...
// TestViewerCanReadProvisioningProfiles is "apply the standard RBAC
// matrix", applied to ProvisioningProfiles: Viewer can read.
func TestViewerCanReadProvisioningProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteProvisioningProfiles is "apply the standard RBAC
// matrix", applied to ProvisioningProfiles: Viewer cannot write.
func TestViewerCannotWriteProvisioningProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteProvisioningProfiles is "apply the standard RBAC
// matrix", applied to ProvisioningProfiles: Operator can write.
func TestOperatorCanWriteProvisioningProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteProvisioningProfiles is "apply the standard
// RBAC matrix", applied to ProvisioningProfiles: Administrator can
// write.
func TestAdministratorCanWriteProvisioningProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeProvisioningProfileService(), expiredValidator, auth.RoleAdministrator)
