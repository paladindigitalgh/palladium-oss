...
//
// Mirrors internal/product/postgres/errors.go's translateError exactly,
// including the foreign-key-violation branch:
// provisioning_profiles.product_id references products(id) ON DELETE
// RESTRICT, so a foreign key violation is a real, reachable outcome here
// — creating a ProvisioningProfile with a ProductID that does not exist.
// The unique-violation branch covers both of this table's unique
// constraints (product_id+vendor, and vendor+profile_name) — see the
// migration for why both exist.
