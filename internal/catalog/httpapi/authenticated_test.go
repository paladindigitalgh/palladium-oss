...
// newAuthenticatedTestRouter mounts CatalogHandler behind the real
// auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/catalogs in production — the
...
// catalog_handler_test.go's: those test the handler in isolation, with no
// middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeCatalogService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeCatalogService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/catalogs/", nil))
...
// TestViewerCanReadCatalog is "apply the standard RBAC matrix", applied to
// Catalogs: Viewer can read.
func TestViewerCanReadCatalog(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeCatalogService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteCatalog is "apply the standard RBAC matrix",
// applied to Catalogs: Viewer cannot write.
func TestViewerCannotWriteCatalog(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeCatalogService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteCatalog is "apply the standard RBAC matrix", applied
// to Catalogs: Operator can write.
func TestOperatorCanWriteCatalog(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeCatalogService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteCatalog is "apply the standard RBAC matrix",
// applied to Catalogs: Administrator can write.
func TestAdministratorCanWriteCatalog(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeCatalogService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeCatalogService(), expiredValidator, auth.RoleAdministrator)
