...
// the catalogs table itself references another table: products.catalog_id
// references catalogs(id) ON DELETE RESTRICT (see
// internal/product/postgres and database/migrations/00012_product_products.sql),
// so deleting a ProductCatalog that still has Products is a real,
// reachable outcome against this schema. This is the same lesson learned
// the hard way in internal/customer/postgres/errors.go, which originally
// omitted this branch because nothing referenced customers at the time it
// was written, and had to be corrected once internal/location introduced
// a foreign key into it — catalog's errors.go is written with that FK
// from the start rather than waiting to be caught by a failing test.
//
