...
// it for. Mirrors internal/authentication/httpapi/authenticated_test.go's
...
// newAuthenticatedTestRouter mounts ConnectionProfileHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/connection-profiles in
// production — using RequireConnectionProfilesRead/RequireConnectionProfilesWrite.
...
// connection_profile_handler_test.go's: those test the handler in
// isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeConnectionProfileService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/connection-profiles", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireConnectionProfilesRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireConnectionProfilesWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/connection-profiles/", nil))
...
// TestViewerCanReadConnectionProfiles is "apply the standard RBAC
// matrix", applied to Connection Profiles: Viewer can read.
func TestViewerCanReadConnectionProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteConnectionProfiles is "apply the standard RBAC
// matrix", applied to Connection Profiles: Viewer cannot write.
func TestViewerCannotWriteConnectionProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteConnectionProfiles is "apply the standard RBAC
// matrix", applied to Connection Profiles: Operator can write.
func TestOperatorCanWriteConnectionProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteConnectionProfiles is "apply the standard
// RBAC matrix", applied to Connection Profiles: Administrator can
// write.
func TestAdministratorCanWriteConnectionProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeConnectionProfileService(), expiredValidator, auth.RoleAdministrator)
