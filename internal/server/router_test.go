...
// technique stubServiceService and every other stub above uses.
...
// newRouterWithProviders mirrors newRouterWithServices exactly, one
// domain over: it proves /api/v1/providers is wired up behind
// auth.Middleware and authz.Middleware in the real production router,
...
// share Catalog's capability pair). See
...
// newRouterWithDiagnostics mirrors newRouterWithProviders exactly,
...
// newRouterWithAuthentication mirrors newRouterWithProviders exactly,
// one domain over: it proves /api/v1/authentication-methods is
// wired up behind auth.Middleware and authz.Middleware in the real
