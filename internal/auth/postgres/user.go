...
// must already be a bcrypt hash produced by auth.HashPassword; the
...
func (r *UserRepository) Create(ctx context.Context, user auth.User) (auth.User, error) {
	...
	now := r.clock.Now()
	created, err := scanUser(r.db.QueryRow(ctx, query, r.ids.New(), user.Email, user.PasswordHash, now))
	if err != nil {
