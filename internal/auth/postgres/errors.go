...
// violation (users.email is UNIQUE). See:
...
// with one difference: there is no foreign-key-violation branch here,
// because the users table has no foreign keys. Handling an error code that
// can never occur against this schema would be dead code, not defensive
// programming.
//
// Callers check for pgx.ErrNoRows themselves before calling this function,
// for the same reason as the Inventory repositories: "no rows" is an
// expected outcome translated to apperror.NotFound with a message naming
// what was missing, not a failure this generic translator should guess a
// message for.
func translateError(op string, err error) error {
	...
	if errors.As(err, &pgErr) && pgErr.Code == pgUniqueViolation {
		return apperror.Conflict(fmt.Sprintf("%s: email already registered", op))
	}
