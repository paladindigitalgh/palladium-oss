...
// newAuthenticatedTestRouter mounts AuthenticationHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/authentication-methods in
// production — using RequireAuthenticationRead/RequireAuthenticationWrite.
...
// authentication_handler_test.go's: those test the handler in
// isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeAuthenticationService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/authentication-methods", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireAuthenticationRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireAuthenticationWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/authentication-methods/", nil))
...
// TestViewerCanReadAuthenticationMethods is "apply the standard RBAC
// matrix", applied to Authentication: Viewer can read.
func TestViewerCanReadAuthenticationMethods(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteAuthenticationMethods is "apply the standard
// RBAC matrix", applied to Authentication: Viewer cannot write.
func TestViewerCannotWriteAuthenticationMethods(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteAuthenticationMethods is "apply the standard RBAC
// matrix", applied to Authentication: Operator can write.
func TestOperatorCanWriteAuthenticationMethods(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteAuthenticationMethods is "apply the standard
// RBAC matrix", applied to Authentication: Administrator can write.
func TestAdministratorCanWriteAuthenticationMethods(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAuthenticationService(), expiredValidator, auth.RoleAdministrator)
