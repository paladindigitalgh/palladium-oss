...
// it for. Mirrors internal/service/httpapi/authenticated_test.go's stub
// of the same name.
...
// newAuthenticatedTestRouter mounts ServiceEquipmentHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/service-equipment in
...
func newAuthenticatedTestRouter(svc *fakeServiceEquipmentService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/service-equipment", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireServiceEquipmentRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireServiceEquipmentWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/service-equipment/", nil))
...
// TestViewerCanReadServiceEquipment is "apply the standard RBAC matrix",
// applied to Service Equipment: Viewer can read.
func TestViewerCanReadServiceEquipment(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteServiceEquipment is "apply the standard RBAC
// matrix", applied to Service Equipment: Viewer cannot write.
func TestViewerCannotWriteServiceEquipment(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteServiceEquipment is "apply the standard RBAC
// matrix", applied to Service Equipment: Operator can write.
func TestOperatorCanWriteServiceEquipment(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteServiceEquipment is "apply the standard RBAC
// matrix", applied to Service Equipment: Administrator can write.
func TestAdministratorCanWriteServiceEquipment(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeServiceEquipmentService(), expiredValidator, auth.RoleAdministrator)
