...
//
// This mirrors internal/service/postgres/errors.go's translateError —
// including the foreign-key-violation branch, written in from the start
// (see that file's own doc comment on why): service_equipment.service_id
// and service_equipment.device_id both reference their parent tables ON
// DELETE RESTRICT, so a foreign key violation is a real, reachable
// outcome here in three directions — creating/updating a ServiceEquipment
// with a ServiceID or DeviceID that does not exist, and deleting a
// Service or Device that still has a ServiceEquipment record.
//
