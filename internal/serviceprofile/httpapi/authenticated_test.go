...
// newAuthenticatedTestRouter mounts ServiceProfileHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/service-profiles in
// production — using RequireServiceProfilesRead/RequireServiceProfilesWrite,
// a dedicated capability pair, not a reuse of Catalog's or Product's (see
// authz.CanReadServiceProfiles's doc comment). The fake service and stub
// user repository are the only stand-ins; everything about how a request
...
// tests from service_profile_handler_test.go's: those test the handler
// in isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeServiceProfileService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/service-profiles", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireServiceProfilesRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireServiceProfilesWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/service-profiles/", nil))
...
// TestViewerCanReadServiceProfiles is "apply the standard RBAC matrix",
// applied to Service Profiles: Viewer can read.
func TestViewerCanReadServiceProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteServiceProfiles is "apply the standard RBAC
// matrix", applied to Service Profiles: Viewer cannot write.
func TestViewerCannotWriteServiceProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteServiceProfiles is "apply the standard RBAC
// matrix", applied to Service Profiles: Operator can write.
func TestOperatorCanWriteServiceProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteServiceProfiles is "apply the standard RBAC
// matrix", applied to Service Profiles: Administrator can write.
func TestAdministratorCanWriteServiceProfiles(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeServiceProfileService(), expiredValidator, auth.RoleAdministrator)
