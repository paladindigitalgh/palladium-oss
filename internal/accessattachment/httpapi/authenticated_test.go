...
// it for. Mirrors internal/accessinterface/httpapi/authenticated_test.go's
...
// newAuthenticatedTestRouter mounts AccessAttachmentHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/access-attachments in
// production — using RequireAccessTopologyRead/RequireAccessTopologyWrite,
// the same capability pair /api/v1/access-interfaces uses (see
// authz.CanReadAccessTopology's doc comment for why AccessAttachment
// does not get its own dedicated pair). The fake service and stub user
...
// tests from access_attachment_handler_test.go's: those test the
// handler in isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeAccessAttachmentService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/access-attachments", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireAccessTopologyRead())
			r.Get("/", handler.List)
			r.Get("/{id}", handler.Get)
		})

		r.Group(func(r chi.Router) {
			r.Use(authzMiddleware.RequireAccessTopologyWrite())
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/access-attachments/", nil))
...
// TestViewerCanReadAccessAttachments is "apply the standard RBAC
// matrix", applied to Access Attachments: Viewer can read.
func TestViewerCanReadAccessAttachments(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteAccessAttachments is "apply the standard RBAC
// matrix", applied to Access Attachments: Viewer cannot write.
func TestViewerCannotWriteAccessAttachments(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteAccessAttachments is "apply the standard RBAC
// matrix", applied to Access Attachments: Operator can write.
func TestOperatorCanWriteAccessAttachments(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteAccessAttachments is "apply the standard RBAC
// matrix", applied to Access Attachments: Administrator can write.
func TestAdministratorCanWriteAccessAttachments(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAccessAttachmentService(), expiredValidator, auth.RoleAdministrator)
