...
//
// This mirrors internal/oltmodel/postgres/errors.go's translateError:
// the unique-violation branch is reachable via
// idx_device_manufacturers_name (creating/updating a second
// DeviceManufacturer with the same name), and the foreign-key-violation
// branch is reachable as the parent side of
// device_models.manufacturer_id ON DELETE RESTRICT (see
// database/migrations/00034_devicemanufacturer_device_manufacturers.sql)
// — deleting a DeviceManufacturer that still has DeviceModels
// referencing it.
