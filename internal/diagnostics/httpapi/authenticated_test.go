...
// newAuthenticatedTestRouter mounts DiagnosticsHandler behind the real
// auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/diagnostics in production —
// using RequireDiagnostics, the single capability guarding this
// endpoint (see authz.CanRunDiagnostics's doc comment for why there is
// no separate read/write split here). The fake service and stub user
...
// tests from diagnostics_handler_test.go's: those test the handler in
// isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeDiagnosticsService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
	...
	r.Route("/diagnostics", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Post("/basic-onu-check", handler.BasicONUCheck)
		})
	})
	return r
}
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(&fakeDiagnosticsService{result: placeholderResult()}, tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/diagnostics/basic-onu-check", strings.NewReader(validBody)))
...
// TestViewerCannotRunDiagnostics is "apply the standard RBAC matrix",
// applied to Diagnostics: Viewer cannot run — see
// authz.CanRunDiagnostics's doc comment for why running a diagnostic is
// treated as an action, not a read.
func TestViewerCannotRunDiagnostics(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(&fakeDiagnosticsService{result: placeholderResult()}, tokens, auth.RoleViewer)
...
// TestOperatorCanRunDiagnostics is "apply the standard RBAC matrix",
// applied to Diagnostics: Operator can run.
func TestOperatorCanRunDiagnostics(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(&fakeDiagnosticsService{result: placeholderResult()}, tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanRunDiagnostics is "apply the standard RBAC
// matrix", applied to Diagnostics: Administrator can run.
func TestAdministratorCanRunDiagnostics(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(&fakeDiagnosticsService{result: placeholderResult()}, tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(&fakeDiagnosticsService{result: placeholderResult()}, expiredValidator, auth.RoleAdministrator)
