...
// olt_handler_test.go's: those test the handler in isolation, with no
// middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeOLTService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeOLTService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/olts/", nil))
...
// TestViewerCanReadOLTs is "apply the standard RBAC matrix", applied to
// OLTs: Viewer can read.
func TestViewerCanReadOLTs(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeOLTService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteOLTs is "apply the standard RBAC matrix", applied
// to OLTs: Viewer cannot write.
func TestViewerCannotWriteOLTs(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeOLTService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteOLTs is "apply the standard RBAC matrix", applied
// to OLTs: Operator can write.
func TestOperatorCanWriteOLTs(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeOLTService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteOLTs is "apply the standard RBAC matrix",
// applied to OLTs: Administrator can write.
func TestAdministratorCanWriteOLTs(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeOLTService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeOLTService(), expiredValidator, auth.RoleAdministrator)
