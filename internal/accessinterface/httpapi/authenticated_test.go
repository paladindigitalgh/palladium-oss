...
// newAuthenticatedTestRouter mounts AccessInterfaceHandler behind the
// real auth.Middleware and authz.Middleware, exactly as
// internal/server/router.go wires /api/v1/access-interfaces in
// production — using RequireAccessTopologyRead/RequireAccessTopologyWrite,
// the same capability pair /api/v1/access-attachments uses (see
// authz.CanReadAccessTopology's doc comment for why AccessInterface does
// not get its own dedicated pair). The fake service and stub user
...
// tests from access_interface_handler_test.go's: those test the handler
// in isolation, with no middleware in front of it at all.
func newAuthenticatedTestRouter(svc *fakeAccessInterfaceService, tokens *auth.TokenIssuer, role auth.Role) http.Handler {
...
func TestUnauthenticatedRequestRejectedWithoutReachingHandler(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), tokens, auth.RoleAdministrator)

	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/access-interfaces/", nil))
...
// TestViewerCanReadAccessInterfaces is "apply the standard RBAC matrix",
// applied to Access Interfaces: Viewer can read.
func TestViewerCanReadAccessInterfaces(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), tokens, auth.RoleViewer)
...
// TestViewerCannotWriteAccessInterfaces is "apply the standard RBAC
// matrix", applied to Access Interfaces: Viewer cannot write.
func TestViewerCannotWriteAccessInterfaces(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), tokens, auth.RoleViewer)
	token := mustIssueToken(t, tokens)
...
// TestOperatorCanWriteAccessInterfaces is "apply the standard RBAC
// matrix", applied to Access Interfaces: Operator can write.
func TestOperatorCanWriteAccessInterfaces(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), tokens, auth.RoleOperator)
	token := mustIssueToken(t, tokens)
...
// TestAdministratorCanWriteAccessInterfaces is "apply the standard RBAC
// matrix", applied to Access Interfaces: Administrator can write.
func TestAdministratorCanWriteAccessInterfaces(t *testing.T) {
	tokens := auth.NewTokenIssuer([]byte("test-secret"), time.Hour, clock.NewFrozen(authTestNow))
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), tokens, auth.RoleAdministrator)
	token := mustIssueToken(t, tokens)
...
func TestAuthenticatedEndpointRejectsExpiredToken(t *testing.T) {
	...
	router := newAuthenticatedTestRouter(newFakeAccessInterfaceService(), expiredValidator, auth.RoleAdministrator)
