...
// including the foreign-key-violation branch: products.catalog_id
// references catalogs(id) ON DELETE RESTRICT, so a foreign key violation
// is a real, reachable outcome here — creating/updating a Product with a
// CatalogID that does not exist. (Deleting a Catalog that still has
// Products is the same violation from the other direction, handled by
// internal/catalog/postgres's own translateError.) It maps to
// apperror.KindConflict for the same reasoning given there: the request
// conflicts with the current relational state of the data.
//
