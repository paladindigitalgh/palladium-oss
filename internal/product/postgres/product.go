...
func (r *ProductRepository) Get(ctx context.Context, productID uuid.UUID) (product.Product, error) {
	const query = `
		SELECT id, catalog_id, provider_id, name, category, status, created_at, updated_at
		FROM products
		WHERE id = $1
	`
...
func (r *ProductRepository) Create(ctx context.Context, p product.Product) (product.Product, error) {
	const query = `
		INSERT INTO products (id, catalog_id, provider_id, name, category, status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $7)
		RETURNING id, catalog_id, provider_id, name, category, status, created_at, updated_at
	`

	now := r.clock.Now()
	created, err := scanProduct(r.db.QueryRow(ctx, query,
		r.ids.New(), p.CatalogID, p.ProviderID, p.Name, string(p.Category), string(p.Status), now))
	if err != nil {
...
func (r *ProductRepository) Update(ctx context.Context, p product.Product) (product.Product, error) {
	const query = `
		UPDATE products
		SET catalog_id = $1, provider_id = $2, name = $3, category = $4, status = $5, updated_at = $6
		WHERE id = $7
		RETURNING id, catalog_id, provider_id, name, category, status, created_at, updated_at
	`

	updated, err := scanProduct(r.db.QueryRow(ctx, query,
		p.CatalogID, p.ProviderID, p.Name, string(p.Category), string(p.Status), r.clock.Now(), p.ID))
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func scanProduct(row rowScanner) (product.Product, error) {
	...
	err := row.Scan(&p.ID, &p.CatalogID, &p.ProviderID, &p.Name, &category, &status, &p.CreatedAt, &p.UpdatedAt)
