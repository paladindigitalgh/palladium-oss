...
func (r *ProductRepository) Get(ctx context.Context, productID uuid.UUID) (product.Product, error) {
	const query = `
		SELECT id, catalog_id, provider_id, name, category, service_type, status, created_at, updated_at
		FROM products
		WHERE id = $1
	`
...
func (r *ProductRepository) Create(ctx context.Context, p product.Product) (product.Product, error) {
	const query = `
		INSERT INTO products (id, catalog_id, provider_id, name, category, service_type, status, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $8)
		RETURNING id, catalog_id, provider_id, name, category, service_type, status, created_at, updated_at
	`

	now := r.clock.Now()
	created, err := scanProduct(r.db.QueryRow(ctx, query,
		r.ids.New(), p.CatalogID, p.ProviderID, p.Name, string(p.Category), string(p.ServiceType), string(p.Status), now))
	if err != nil {
...
func (r *ProductRepository) Update(ctx context.Context, p product.Product) (product.Product, error) {
	const query = `
		UPDATE products
		SET catalog_id = $1, provider_id = $2, name = $3, category = $4, service_type = $5, status = $6, updated_at = $7
		WHERE id = $8
		RETURNING id, catalog_id, provider_id, name, category, service_type, status, created_at, updated_at
	`

	updated, err := scanProduct(r.db.QueryRow(ctx, query,
		p.CatalogID, p.ProviderID, p.Name, string(p.Category), string(p.ServiceType), string(p.Status), r.clock.Now(), p.ID))
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func scanProduct(row rowScanner) (product.Product, error) {
	...
	err := row.Scan(&p.ID, &p.CatalogID, &p.ProviderID, &p.Name, &category, &serviceType, &status, &p.CreatedAt, &p.UpdatedAt)
	p.Category = product.ProductCategory(category)
	p.ServiceType = product.ServiceType(serviceType)
