...
- [x] Retry logic
- [x] Audit trail (Event domain). Originally Workflow-transitions-only ("Workflow started"/"succeeded"/"failed"); expanded 2026-09-11 (user's explicit request, Recent Activity's generic text "wasn't particularly useful") so Device/Customer/Contact/Location/Service/Service-Equipment/Customer-Device handlers also write real, human-readable Events directly -- e.g. "Attached device test-05 to Acme Corp's service" -- resolving cross-entity names (Customer/Location/Device) at the httpapi handler layer, never inside the domain services themselves, mirroring Notes' own author-name-resolution precedent. `GET /events` also gained an unbounded "list everything" mode (both entity_type/entity_id omitted) to back the new Explorer Activity page
